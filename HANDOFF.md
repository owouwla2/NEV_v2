...
> **创建日期**：2026-05-15
> **最后更新**：2026-05-24
> **当前进度**：🏆 **首版交付完成**（tag `wave-4-end`）—— 后端 38 模块 + 前端 12 业务页 + 5 角色端到端 e2e 全验证通过；文档/演示视频留答辩前补
...
### 2.34 Wave 3 D17 完成清单（2026-05-23）

### 2.37 Wave 3 D18 完成清单（2026-05-23）

| # | 任务 | 结果 |
|---|---|---|
| 1 | 新建 `backend/nev-modules/nev-carbon` Maven 子模块（依赖 nev-battery + ruoyi-common-*） + 注册到 nev-modules/pom.xml + 引入 ruoyi-admin/pom.xml | ✓ |
| 2 | 5 个实体（继承 BaseEntity + @TableLogic）：`NevCarbonFootprintDO`（1:1 nev_battery，全生命周期总碳排）/ `NevCarbonStageDO`（5 阶段明细 RAW/MFG/TRANS/USE/EOL）/ `NevEmissionFactorDO`（D5-D6 灌 15 条数据）/ `NevCarbonCreditAccountDO`（每用户碳积分账户）/ `NevCarbonCreditRecordDO`（碳积分流水） | ✓ |
| 3 | 5 个 Mapper（BaseMapper 即可） | ✓ |
| 4 | `CarbonAutoConfiguration` @ComponentScan + @MapperScan | ✓ |
| 5 | `mvn -q clean install -DskipTests -T 4` 全编译通过（模块数 36 → **37**） | ✓ |

### 2.38 D18 设计要点

- **碳计算遵循 GB-T 24067（中国国标）框架**：calc_method 字段默认 `GB-T-24067`，老仓 NEV/backend 的 CarbonCalculationService 也按此标准。calc_version 字段预留因子库或算法更新后的版本号变更（升级时新计算结果不覆盖旧记录，而是写新版本）。
- **5 阶段固定 enum 值**：`RAW`（原材料）/ `MFG`（制造）/ `TRANS`（运输）/ `USE`（使用）/ `EOL`（报废）。代码用 String 常量而非 Java enum，便于跨模块共享 + 数据库直接可读。
- **breakdown JSON 设计**：nev_carbon_stage.breakdown 字段是 JSON 数组，记录各排放源 + 引用的 factor_code + 投入量 + 小计。例：`[{"factorCode":"RAW-LITHIUM","input":3.5,"unit":"kg","co2":52.5},...]`。这样消费者扫码看到的不只是"总数"还能看到"为什么"。
- **积分单位 = kgCO2eq**：碳积分账户余额单位用 kgCO2eq（千克二氧化碳当量），与排放因子单位对齐。1 kgCO2eq 积分 = 1 kg 碳减排贡献。后续可对接政府碳普惠。
- **跨模块依赖链**：nev-carbon → nev-battery → nev-blockchain。D19 算法用 NevBatteryMapper（在 nev-battery）查电池规格，D20 公开扫码依赖 BatteryScanService 扩展。

### 2.39 D18 待办（D19-D20）

- ⏸ **D19** `CarbonCalculationService`：5 阶段算法（参考老仓 `NEV/backend/.../CarbonCalculationService.java`）；按电池 spec（capacity_kwh / model / cellType 等）+ 排放因子库逐阶段累加；写 nev_carbon_footprint + 5 行 nev_carbon_stage；admin 端点 POST `/admin/carbon/calc/{batteryId}` 手工触发；DBA 端点 GET `/public/carbon/{traceNumber}`
- ⏸ **D20** 扫码 VO 加 `carbonFootprint` 字段（total + 5 stages + breakdown）；消费者 confirm 订单时自动按订单含 battery 商品给 consumer 加 `change_amount = battery 减排量` 积分；查询积分账户端点 GET `/consumer/carbon/account`



| # | 任务 | 结果 |
|---|---|---|
| 1 | NevPaymentRecordDO 实体（对应 nev_payment_record 表，含 paymentNo/amount/method/status/tradeNo） + NevPaymentRecordMapper | ✓ |
| 2 | nev-marketplace/pom.xml 加 nev-battery 依赖；BatteryService 新增 public `recordSoldByMerchant(traceNumber, merchantUserId, merchantWallet, orderNo, consumerId)` 跨模块入口，不走 LoginHelper，由调用方传入 operator 上下文 | ✓ |
| 3 | OrderService 新增 3 方法：pay (PENDING→PAID + 写 nev_payment_record method=MOCK)；ship (PAID→SHIPPED，merchant 校验)；confirm (SHIPPED→COMPLETED + 找含 batteryId 的 order_item → 经 BatteryService.recordSoldByMerchant 触发链上 SOLD) | ✓ |
| 4 | findTraceNumberByBatteryId() helper 通过 nev-battery 模块的 NevBatteryMapper 反查 trace_number（缓存到 ConcurrentHashMap） | ✓ |
| 5 | NevCartItemMapper 加 `physicalDeleteByIds`（@Delete 注解原生 SQL，绕过 @TableLogic）—— 修订 D16 bug：cart_item 逻辑删除导致 (cart_id, product_id) 唯一索引复用失败 | ✓ |
| 6 | ConsumerOrderController 加 POST `/consumer/order/pay/{id}` + `/confirm/{id}`；MerchantOrderController 加 POST `/merchant/order/ship/{id}` | ✓ |
| 7 | `mvn -q clean install -DskipTests -T 4` 全编译通过（36 模块） | ✓ |
| 8 | **修订 D15 数据 bug**：之前商品绑定的 batteryId 是 lifecycle 行 id 而非 nev_battery 行 id；用 SQL 把 `nev_product.battery_id` 从 `2058073529739493377` 改为 `2058084464755515393`（BAT-DEMO-002 真实 battery id） | ✓ |
| 9 | smoke test：consumer1 加 BAT-DEMO-002 商品入购物车 → 下单 → pay (PENDING→PAID + paymentRecord) → merchant1 ship (PAID→SHIPPED) → consumer1 confirm (SHIPPED→COMPLETED) | ✓ |
| 10 | DB 验证：nev_battery_lifecycle 多 1 行 **v=5 SOLD operator_id=104 operator_role=merchant**，data_hash + tx_hash 写入 | ✓ |
| 11 | **公开扫码 /public/scan/BAT-DEMO-002**：overallVerified=True，totalEvents=5，verifiedEvents=5，**chainEventCount=5**，所有 5 events（PRODUCED v1 → IN_USE v2 → SOLD v3 by retailer → RECYCLED v4 → SOLD v5 by merchant）chainVerified 全 True | ✓ |

### 2.35 D17 关键发现

- **商城 SOLD vs 零售商 SOLD 共存**：v3 是 D13 retailer1 直接调 `/retailer/battery/sell` 写入；v5 是 D17 商城订单 confirm 时商家代理写入。链上同一 traceNumber 出现 2 个 SOLD 事件，operator 不同。这真实反映了"门店零售"和"商城零售"两种渠道并存——业务上完全合法。
- **跨模块调用模式**：nev-marketplace 依赖 nev-battery，调 `BatteryService.recordSoldByMerchant(...)` 公开入口。该方法**不走 LoginHelper / requireOperator**，由调用方显式传入 operator 上下文（userId + wallet + role）。这是跨模块 service 互调的标准模式。
- **逻辑删除 vs 物理删除的取舍**：cart_item 用 `@TableLogic` 逻辑删除 + `(cart_id, product_id)` 唯一索引 → 重复添加同商品永远撞索引。MyBatis-Plus 没有"包含已删除的查询"开关，最简方案就是订单结算后**物理删除** cart_item。加 `@Delete` 注解原生 SQL 绕过 @TableLogic。
- **batteryId 绑定要绑 nev_battery.id 而非 lifecycle id**：D15 上架时我误把 lifecycle 行 id 当 battery id 写入 product.battery_id。修订靠 SQL `UPDATE nev_product SET battery_id = <real>` 修复。**生产环境应在 ProductService.save 加 `productMapper.selectById(batteryId)` 校验**（D18 补）。
- **状态机闭环**：PENDING（下单）→ PAID（mock 支付）→ SHIPPED（商家发货）→ COMPLETED（消费者确认收货 + 链上 SOLD）→ 终态。CANCELLED 仅从 PENDING 转入。退款（REFUNDED）留 Wave 3+。
- **TableId 类型选 ASSIGN_ID**：MyBatis-Plus 雪花 ID 生成 Long 类型，但序列化成 JSON 后是数字。consumer 端的 JSON 反序列化要小心精度——前端 JS Number 不能精确表示 > 2^53 的 Long，建议在 RuoYi 全局 Jackson 配置加 `Long → String`。本轮 demo 测试用 Python 没踩到，前端集成时若出问题需调整。

### 2.36 D17 后链上状态快照

```
BAT-DEMO-002 链上 lifecycleEvents (5 events 全 verified):
  v=1 PRODUCED   producer1 (101)  via /producer/battery/register
  v=2 IN_USE     distrib1  (102)  via /distributor/battery/transfer-in
  v=3 SOLD       retailer1 (103)  via /retailer/battery/sell           (老路径)
  v=4 RECYCLED   recycler1 (106)  via /recycler/battery/receive
  v=5 SOLD       merchant1 (104)  via /consumer/order/confirm 自动联动 (新路径，商城闭环)
```
...
| # | 任务 | 结果 |
|---|---|---|
| 1 | 4 个实体：NevCartDO / NevCartItemDO / NevOrderDO / NevOrderItemDO（继承 BaseEntity） + 4 个 BaseMapper | ✓ |
| 2 | 7 个 DTO/VO：CartAddDTO / CartItemUpdateDTO / CartVO（按 merchant 分组 + 整体小计） / AddressDTO / OrderCreateDTO / OrderVO（含 item 列表 + 地址 + 状态机时间戳） / OrderQueryDTO | ✓ |
| 3 | CartService：add（已有商品累加） / listMine（按 merchant 分组 + 批量查商品+商家） / update（数量+selected） / remove / clear。每用户每商家一个 cart 懒创建。 | ✓ |
| 4 | OrderService：createFromCart（同商家校验+库存扣减+雪花订单号 ORDyyyyMMddHHmmss+随机+地址 JSON 快照+商品 JSON 快照+清空已下单 cart_item） / listMineConsumer / listMineMerchant / detail（双端校验） / cancel（仅 PENDING 可取消，**还原库存** + sales_count） | ✓ |
| 5 | 3 个 Controller：ConsumerCartController（5 端点 add/list/update/remove/clear） / ConsumerOrderController（4 端点 create/list/detail/cancel） / MerchantOrderController（2 端点 list/detail） | ✓ |
| 6 | `mvn -q clean install -DskipTests -T 4` 全编译通过（36 模块） | ✓ |
| 7 | smoke test：consumer1 加 2 个商品 → 购物车 grandSelectedTotal=35997.00 ✓ | ✓ |
| 8 | smoke test：consumer1 下单 → orderNo=ORD..., status=PENDING, totalAmount=35997, items=2，商品快照含 batteryId | ✓ |
| 9 | smoke test：下单后购物车自动清空 + 商品库存 10→9 / 50→48 | ✓ |
| 10 | smoke test：merchant1 查自家订单 total=1 看到 consumer1 的订单 | ✓ |
| 11 | smoke test：consumer1 取消订单 → status=CANCELLED + cancelReason 写入 + **库存自动还原** 10/50 | ✓ |
| 12 | 状态机反例：已 CANCELLED 订单再次取消返回 code=500 + msg=`订单当前状态 [CANCELLED] 不可取消` | ✓ |

### 2.33 D16 关键设计

- **同商家约束**：一次下单的所有 cart_item 必须来自同一商家。否则 `throw ServiceException("一次下单的商品必须来自同一商家")`。前端结算时也按 merchant 分多次下单。
- **雪花订单号格式**：`ORD${yyyyMMddHHmmss}${random6}`，业务唯一 + 时间可读。MyBatis-Plus 自带的 ID 雪花用于 `id`（主键），订单号是业务字段单独生成。
- **JSON 快照防失真**：order_item.product_snapshot 在下单时定格 title/category/price/batteryId/images，后续商品改名或下架不影响历史订单展示。
- **取消状态机**：当前仅允许 `PENDING → CANCELLED`（未支付才能取消）。已支付订单进入退款流程留 D17。
- **库存事务一致**：createFromCart / cancel 都 @Transactional，库存扣减/还原与订单状态写入要么全成功要么全回滚。
- **Sa-Token 鉴权严格分角色**：consumer 操作购物车和自己的订单；merchant 只能看自家订单（不能改）。其他角色（producer/distributor 等）调任意 /consumer/* /merchant/* 端点 → 403。
- **图片字段三态**：DB 存 JSON 字符串 / VO 反序列化为 `List<String>` / DTO 接收 `List<String>`。所有 marketplace 模块统一这套约定。



| # | 任务 | 结果 |
|---|---|---|
| 1 | 新建 `backend/nev-modules/nev-marketplace` Maven 子模块 + 注册到 nev-modules/pom.xml + 引入 ruoyi-admin/pom.xml | ✓ |
| 2 | `NevMerchantDO` + `NevProductDO` 实体 + 2 个 BaseMapper（继承 BaseEntity 用字段自动填充） | ✓ |
| 3 | `ProductSaveDTO` 含 bean-validation（title 非空 + price > 0 + stock >= 0 + 可选 batteryId） + `ProductVO` + `ProductQueryDTO`（分页 + 类目/关键词/商家过滤） | ✓ |
| 4 | `ProductService` 含 4 个公共方法：save / listMine / publicSearch / ensureMerchantProfile（merchant 首次操作自动建档案） | ✓ |
| 5 | images 字段用 Jackson 序列化为 JSON 数组字符串存 DB，VO 反序列化为 List<String> | ✓ |
| 6 | `MerchantProductController` @SaCheckRole("merchant")：POST /merchant/product/save + GET /merchant/product/mine | ✓ |
| 7 | `PublicProductController` 无鉴权：GET /public/product/list?category=&keyword=&pageNum=&pageSize=（MyBatis-Plus 分页） | ✓ |
| 8 | `mvn -q clean install -DskipTests -T 4` 通过，模块数 35 → **36** | ✓ |
| 9 | smoke test：merchant1 登录 → 2 次 POST /merchant/product/save 上架 BATTERY（绑 batteryId=2058073529739493377）+ ACCESSORY 商品；自动建 nev_merchant(user_id=104, status=ACTIVE) 档案 | ✓ |
| 10 | smoke test：GET /public/product/list 返回 total=2 含 merchant_name="merchant-104" + images 数组；?category=BATTERY 返回 total=1 | ✓ |
| 11 | 鉴权反例：未登录 401 ✓；producer1 token 调 /merchant/* → 403 ✓ | ✓ |

### 2.31 D15 关键发现

- **merchant 档案自动建档**：`ProductService.ensureMerchantProfile()` 首次操作时自动 INSERT 一行 nev_merchant（merchant_name="merchant-{userId}" 占位）。生产部署时由 admin 后台维护正式档案，本轮 demo 简化。
- **JSON 数组字段策略**：images 用 Jackson `writeValueAsString` 存为 varchar JSON 字符串，VO 用 `TypeReference<List<String>>` 反序列化。其他类似字段（payload、address_snapshot 等）可复用同模式。
- **MyBatis-Plus 自带分页**：`Page<NevProductDO>` + `mapper.selectPage(page, wrapper)`。需要 RuoYi 已注入 PaginationInnerInterceptor（ruoyi-common-mybatis 配置好）。
- **Sa-Token 异常响应**：未登录返回 HTTP 200 + body `{code:401, msg:"认证失败..."}`；角色不匹配返回 HTTP 200 + `{code:403, msg:"没有访问权限..."}`。前端按 body.code 判断。
- **类目字典字段**：商品 `category` 字段对齐 D6 写入的 `sys_dict_data(dict_type='nev_product_category')`：BATTERY / ACCESSORY / SERVICE。本轮没在 service 加字典校验，前端下拉框直接读字典。
